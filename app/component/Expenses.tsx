...
// Define the interface for an Expense record
interface Expense {
  id: number;
  date: string;
  category: string;
...
const ExpenseForm: React.FC = () => {
  const [state, setState] = useState<State>({
    expenses: [],
    form: { date: '', category: '', description: '', amount: 0 },
    editingId: null,
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    if (!form.date || !form.category || !form.description || form.amount <= 0) {
    ...
    try {
      ...
      setState((prev) => ({
        ...
        editingId: null,
        isFormModalAnimating: false,
        currentPage: editingId === null ? Math.ceil((expenses.length + 1) / itemsPerPage) : prev.currentPage,
      }));
      setTimeout(() => {
        setState((prev) => ({ ...prev, showFormModal: false }));
        toast.success(method === 'POST' ? 'Expense added successfully!' : 'Expense updated successfully!');
      }, 300);
    } catch (error: any) {
      console.error('Error submitting expense data:', error);
      toast.error(`Failed to save expense record: ${error.message}`);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await fetchExpenses();
      const totalPagesAfterDelete = Math.ceil((expenses.length - 1) / itemsPerPage);
  ...
  return (
    <div className={`${styles.container} max-w-7xl mx-auto p-4 sm:p-6 md:p-8 bg-[#E6F0FA] min-h-screen font-sans`}>
      ...
      <div className={`${styles.cardShadow} bg-white p-4 sm:p-6 rounded-2xl shadow-xl mb-6 text-center`}>
        <h3 className="text-lg sm:text-xl font-semibold text-gray-700 mb-2">Total Expenses</h3>
        <p className="text-2xl sm:text-4xl font-extrabold text-[#f44336]">
          -Birr {totalExpensesAmount.toFixed(2)}
        </p>
      </div>
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <h2 className="text-xl sm:text-2xl font-bold text-gray-800 mb-6 text-center">
                {editingId ? 'Edit Expense Record' : 'Add New Expense Record'}
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                ...
                <div className="sm:col-span-2">
                  ...
                  <textarea
                    ...
                    id="description"
                    rows={3}
                    placeholder="e.g., Weekly food shopping, Monthly apartment rent"
      ...
      <div className={`${styles.tableContainer} overflow-x-auto bg-white shadow-xl rounded-2xl border border-gray-200 mt-6`}>
            ...
            <tbody>
              {currentItems.length > 0 ? (
                currentItems.map((exp) => (
                  <tr key={exp.id} className="border-b last:border-b-0 border-gray-200">
                    <td className="px-4 sm:px-6 py-3 sm:py-4">{exp.date}</td>
                    <td className="px-4 sm:px-6 py-3 sm:py-4">{exp.category}</td>
                    <td className="px-4 sm:px-6 py-3 sm:py-4">{exp.description}</td>
                    <td className="px-4 sm:px-6 py-3 sm:py-4 text-right font-medium text-[#f44336]">
                ...
                <tr>
                  <td colSpan={5} className="text-center py-6 text-gray-500 italic">
                    {searchTerm ? 'No matching expense records found.' : 'No expense records to display. Add a new expense!'}
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="block sm:hidden">
          {currentItems.length > 0 ? (
            currentItems.map((exp) => (
              <div key={exp.id} className={`${styles.card} border-b border-gray-200 p-4 last:border-b-0`}>
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <p className="font-semibold text-gray-800">Date: {exp.date}</p>
                    <p className="text-gray-600">Category: {exp.category}</p>
                  </div>
                  <p className="font-medium text-right text-[#f44336]">
                    -Birr {exp.amount.toFixed(2)}
                  </p>
                </div>
                <p className="text-gray-600 mb-2">Description: {exp.description}</p>
